...
def main():
    ...
    loss_euc = tf.losses.mean_squared_error(label_onehot, logits)
    loss = loss_euc + loss_reg
    ...
    tf.summary.scalar('loss_euc', loss_euc)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epoch_start+1, config.nr_epoch+1):
            for _ in range(train_set.minibatchs_per_epoch):
                ...
                _, loss_v, loss_reg_v, acc_v, lr_v, summary = sess.run([train, loss_euc, loss_reg,
                                                                        accuracy, lr, merged],
                                                                       feed_dict=feed_dict)
            ...
            if epoch % config.test_interval == 0:
                ...
                for i in range(test_set.minibatchs_per_epoch):
                    ...
                    loss_v, acc_v, summary = sess.run([loss_euc, accuracy, merged],
                                                      feed_dict=feed_dict)
                    loss_sum += loss_v
                    acc_sum += acc_v
